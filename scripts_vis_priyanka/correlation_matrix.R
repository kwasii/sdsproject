...
## get average ratings
gt_tweets$mean_score <- rowMeans(subset(gt_tweets, select = c(daniel, priyanka, florian, dario)))
...
## remove text and categorical columns
df = subset(gt_tweets, select=-c(ï..user_id, status_id, created_at, screen_name, text, lang, location, 
                                  lat, lng, GoogleMapsLocation, country, zip, state))
