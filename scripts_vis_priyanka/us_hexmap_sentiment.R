...
# Load mariage data
elec_res = read.csv("scores_states.csv", sep=';', stringsAsFactors = F)
...
ggplot() +
  geom_polygon(data = spdf_fortified, aes(fill =  meanVader, x = long, y = lat, group = group)) +
...
# Prepare binning
## Syuzhet
#spdf_fortified$bin <- cut( spdf_fortified$meanSyuzhet , breaks=seq(-1.46, 0.66, length.out = 7), labels=c("-1.46: -1.11", "-1.11: -0.75", "-0.75: -0.40", "-0.40: -0.047", "-0.047: 0.31", "0.31: 0.66"), include.lowest = TRUE )

## Vader
spdf_fortified$bin <- cut( spdf_fortified$meanVader , breaks=seq(-0.51, 0.4, length.out = 7), labels=c("-0.51: -0.36", "-0.36: -0.21", "-0.21: -0.055", "-0.055: 0.097", "0.097: 0.25", "0.25: 0.40"), include.lowest = TRUE )
...
ggplot() +
  ...
  scale_fill_manual( 
    values=my_palette, 
    name="Average tweets sentiment for Biden in various US states", 
    guide = guide_legend( keyheight = unit(3, units = "mm"), keywidth=unit(12, units = "mm"), label.position = "bottom", title.position = 'top', nrow=1) 
  ) +
  ggtitle( "2020 USA Election: Average Vader sentiment value per state for Joe Biden" ) +
  theme(
